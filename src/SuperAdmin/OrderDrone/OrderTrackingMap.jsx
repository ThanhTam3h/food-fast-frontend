...
const statusMap = {
	FLYING_TO_PICKUP: {
		text: "Drone đang bay đến nhà hàng lấy món",
		color: "from-orange-500 to-red-600",
	},
	AT_PICKUP_POINT: {
		text: "Đã đến nhà hàng – Đang nhận đồ ăn",
		color: "from-yellow-500 to-amber-600",
	},
	OUT_FOR_DELIVERY: {
		text: "Đã lấy hàng – Đang giao",
		color: "from-blue-500 to-cyan-600",
	},
	DROPPING_OFF: {
		text: "Đang thả đồ ăn xuống!",
		color: "from-red-600 to-rose-700",
	},
	RETURNING_TO_HUB: {
		text: "Đã giao xong – Drone đang quay về trạm",
		color: "from-teal-500 to-emerald-600",
	},
	LANDING_AT_HUB: {
		text: "Drone đã về trạm – Sẵn sàng nhiệm vụ tiếp theo",
...
export default function OrderTrackingMap({ order, drone, onClose }) {
	...
	const restaurant = restaurantCoords
		? [restaurantCoords[0], restaurantCoords[1]]
		: null;
	const customer = customerCoords
	...
	useEffect(() => {
		...
		const map = new goongjs.Map({
			...
			center: [
				(HUB[0] + restaurant[0] + customer[0]) / 3,
				(HUB[1] + restaurant[1] + customer[1]) / 3,
			],
			zoom: 14,
		});
		mapRef.current = map;
		...
		map.on("load", () => {
			...
			new goongjs.Marker({ color: "#8b5cf6" })
				.setLngLat(HUB)
				.setPopup(
					new goongjs.Popup().setHTML(`<span class="font-semibold">HUB</span>`)
				)
				.addTo(map);
			new goongjs.Marker({ color: "#ef4444" })
				...
				.setPopup(
					new goongjs.Popup().setHTML(
						`<span class="font-semibold">${order.merchant.name}</span>`
					)
				)
				.addTo(map);
			...
			map.addSource("route", {
				...
				data: {
					...
					geometry: {
						type: "LineString",
						coordinates: [HUB, restaurant, customer, HUB],
					},
				},
			});
			map.addLayer({
	...
	const statusInfo = droneStatus
		? statusMap[droneStatus]
		: { text: "", color: "from-gray-500 to-gray-700" };
			...
			<div className="bg-white p-6 flex justify-between items-center shadow-2xl">
				<div>
					...
					<p className="text-2xl mt-2">
						Mã đơn: <span className="font-bold">{order.orderNumber}</span>
					</p>
				</div>
				<button
					onClick={onClose}
					className="bg-white text-purple-800 w-16 h-16 rounded-full text-5xl font-bold hover:scale-110 transition">
					×
				</button>
			</div>

			<div
				className={`bg-gradient-to-r ${statusInfo.color} py-6 text-center shadow-xl`}>
				<p className="text-4xl font-bold animate-pulse drop-shadow-2xl">
					{statusInfo.text}
				</p>
			</div>
