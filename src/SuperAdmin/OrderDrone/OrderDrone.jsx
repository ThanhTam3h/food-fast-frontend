		...
		const fetchData = async () => {
			try {
				const [orderRes, droneRes] = await Promise.all([
					fetch("https://food-fast-frontend.vercel.app/api/v1/order"),
					fetch(
						"https://food-fast-frontend.vercel.app/api/v1/order/available-drones"
					),
				]);
	...
	const handleAssignDrone = async () => {
		...
		try {
			const res = await fetch(
				`https://food-fast-frontend.vercel.app/api/v1/order/${assigningOrder.orderNumber}/assign-drone`,
				{
					method: "POST",
