...
export default function AddressPicker({
	...
	const safeDrawDroneLine = (lat, lng) => {
		const map = mapRef.current;
		if (!map) return;

		if (!map.isStyleLoaded()) {
			map.once("load", () => drawDroneLine(lat, lng));
		} else {
	...
	const triggerLocationSelected = (lat, lng, distance, fullAddress) => {
	...
	// ===================== REVERSE GEOCODE =====================
	const reverseGeocode = async (latVal, lngVal, distance) => {
		...
		if (data.status === "OK") {
			...
			triggerLocationSelected(latVal, lngVal, distance, fullAddress);
		}
	};
	...
	const placeCustomerMarker = async (lngVal, latVal) => {
		...
		newMarker.on("dragend", async () => {
			...
			const newDistance = calculateDroneDistance(
				latRestaurant,
				lngRestaurant,
				lat,
				lng
			...
			safeDrawDroneLine(lat, lng);
			await reverseGeocode(lat, lng, newDistance);
		});

		setCustomerMarker(newMarker);
		map.flyTo({ center: [lngVal, latVal], zoom: 15 });
		safeDrawDroneLine(latVal, lngVal);
		await reverseGeocode(latVal, lngVal, distance);
	};
	...
	const handleSelectSuggestion = async (place_id, description) => {
		...
		await reverseGeocode(
			rlat,
			rlng,
			calculateDroneDistance(latRestaurant, lngRestaurant, rlat, rlng).toFixed(
				2
			)
		);
	};
	...
	useEffect(() => {
		...
		if (!restaurant?.address?.location?.coordinates) return;
